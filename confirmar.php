...
//SELECT (ELT(WEEKDAY(fecha) + 1, 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sabado', 'Domingo')) AS DIA, DATE_FORMAT(fecha, "%d/%M/%Y") as CUANDO FROM pedidos
	...
	<div class="container mt-2 p-2">
	    ...
	    <div class="row buscar">
	    	<div class="alert alert-info">En esta seccion solo apareceran las ordenes de la semana actual del mes que aun no han sido confirmadas</div>
	    	<?php 
	    	$consulta = $db->query("SELECT *, (ELT(WEEKDAY(fecha) + 1, 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sabado', 'Domingo')) AS DIA, DATE_FORMAT(fecha, '%d/%M/%Y') as CUANDO FROM pedidos WHERE estado = 0 AND week(`fecha`) = week(now()) and fecha > now() ORDER BY fecha asc");
	    	if (mysqli_num_rows($consulta)==0) {?>
	    		
	    		<div class="alert alert-success col-12 col-lg-6 offset-lg-3 text-center"><h1>Nada para confirmar ;)</h1></div>
				    ...
				    <div class="modal-content">
				      	...
				      	<div class="modal-body text-center">
							<form action="php/camstate.php" method="get">
								...
								<input type="hidden" name="page" value="confirmar.php">
